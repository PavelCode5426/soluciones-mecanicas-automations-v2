...
document.querySelectorAll('.menu-body a').forEach(function (link) {
    ...
    if (!link || !link.href || !pageUrl.startsWith(link.href)) return;
    
    // 1. Marcar el enlace como activo
    link.classList.add('active');
    
    // 2. Activar el elemento de menú directo
    const directLi = link.closest('li');
    if (directLi) {
        directLi.classList.add('menuitem-active');
        directLi.querySelector('.nav-link')?.setAttribute('aria-expanded', "true");
    }
    
    // 3. Expandir todos los menús colapsables padres
    let parent = link.parentElement;
    while (parent && parent !== document.body) {
        // Si es un menú colapsable, expandirlo
        if (parent.classList.contains('collapse')) {
            parent.classList.add('show');
            
            // Activar el li padre de este menú
            const parentLi = parent.closest('li');
            if (parentLi) {
                parentLi.classList.add('menuitem-active');
                parentLi.querySelector('.nav-link')?.setAttribute('aria-expanded', "true");
            }
        }
        
        // Si es un dropdown, activar el toggle
        if (parent.classList.contains('dropdown-menu')) {
            const dropdownToggle = parent.previousElementSibling;
            if (dropdownToggle && dropdownToggle.matches('.dropdown-toggle, .nav-link')) {
